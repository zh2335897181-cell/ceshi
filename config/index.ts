  ...
  const baseConfig: UserConfigExport<'vite'> = {
    ...
    compiler: {
      ...
      vitePlugins: [
        ...
        // React 源码追踪插件 - 仅在开发模式下启用
        ...(process.env.NODE_ENV === 'development' && isH5
          ? [{
              name: 'react-source-tracker',
              transform(code, id) {
                if (id.includes('node_modules')) return null;
                // 为 React 组件添加 _debugSource 属性
                const debugSource = {
                  fileName: id.replace(process.cwd(), ''),
                  lineNumber: 1,
                  columnNumber: 1
                };
                // 这里可以添加更复杂的转换逻辑来注入 _debugSource
                return null; // 暂时返回 null,实际项目中可能需要更复杂的 babel 转换
              },
            }]
          : []),
        ...(isH5
